...
struct MotionTarget {
    ...
    MotionTarget() : baseX(0), baseY(0), baseZ(0), motionMode(MotionMode::JOINT_AND_BASE), taskType(MotionTaskType::TRANSITION), hasProjectorPose(false), projectorPose(Matrix4d::Identity()), planningInfo() {
        jointAngles.setZero();
...
private slots:
    ...
    void onManualViewpointIkSolveClicked();
...
private:
    ...
    std::vector<vtkSmartPointer<vtkBillboardTextActor3D>> worldAxisLabels_;
    std::vector<vtkSmartPointer<vtkBillboardTextActor3D>> baseAxisLabels_;
    ...
    std::vector<Vector6d> manualIkSolutions_;
    Matrix4d manualProjectorPose_;
    double manualIkBaseX_;
    double manualIkBaseY_;
    double manualIkBaseZ_;
    int manualIkSolutionIndex_;
