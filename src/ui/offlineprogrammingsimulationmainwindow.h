...
#include <vtkCell.h>              // VTK单元
#include <vtkMath.h>              // 数学工具
...
struct MotionTarget {
    ...
    MotionTaskType taskType; // 当前运动任务类型
    bool hasProjectorPose; // 是否携带投影器位姿
    Matrix4d projectorPose; // 投影器位姿，列0/1/2分别为Right/Up/Forward
    bool hasStructuredLightGeometry; // 是否携带缓存的结构光几何
    std::array<Vector3d, 5> frustumPoints; // P A B C D
    std::vector<Vector3d> coveredVertices; // 每3个点为一个三角形
    QString planningInfo; // 到达该运动目标时输出的规划信息

    MotionTarget() : baseX(0), baseY(0), baseZ(0), motionMode(MotionMode::JOINT_AND_BASE), taskType(MotionTaskType::TRANSITION), hasProjectorPose(false), projectorPose(Matrix4d::Identity()), hasStructuredLightGeometry(false), planningInfo() {
        jointAngles.setZero();
        frustumPoints.fill(Vector3d::Zero());
    }

    MotionTarget(const Vector6d& angles,
                 double x = 0,
                 double y = 0,
                 double z = 0,
                 MotionMode mode = MotionMode::JOINT_AND_BASE,
                 MotionTaskType task = MotionTaskType::TRANSITION,
                 const Matrix4d* pose = nullptr,
                 const std::array<Vector3d, 5>* frustum = nullptr,
                 const std::vector<Vector3d>* covered = nullptr,
        ...
        : jointAngles(angles),
          ...
          motionMode(mode),
          taskType(task),
          hasProjectorPose(pose != nullptr),
          projectorPose(pose ? *pose : Matrix4d::Identity()),
          hasStructuredLightGeometry(frustum != nullptr && covered != nullptr),
          frustumPoints(frustum ? *frustum : std::array<Vector3d, 5>{}),
          coveredVertices(covered ? *covered : std::vector<Vector3d>{}),
          planningInfo(info) {}
};
...
private:
    ...
    MotionTaskType currentMotionTaskType_;
    bool currentHasProjectorPose_;
    Matrix4d currentProjectorPose_;
    bool currentHasStructuredLightGeometry_;
    std::array<Vector3d, 5> currentFrustumPoints_;
    std::vector<Vector3d> currentCoveredVertices_;
    ...
    vtkSmartPointer<vtkPolyData> workpiecePolyData_;
    vtkSmartPointer<vtkActor> halfFrustumActor_;
    vtkSmartPointer<vtkActor> halfFrustumLineActor_;
    vtkSmartPointer<vtkActor> structuredLightCoveredActor_;
    vtkSmartPointer<vtkActor> projectorPointActor_;
    vtkSmartPointer<vtkActor> projectorDirectionActor_;
    vtkSmartPointer<vtkCamera> structuredLightProjectorCamera_;
    bool structuredLightActorsAdded_;
    bool hasLastStructuredLightProjectorPose_;
    Matrix4d lastStructuredLightProjectorPose_;
    double structuredLightFovY_;
    double structuredLightAspectRatio_;
    double structuredLightNearDist_;
    double structuredLightFarDist_;
    double structuredLightCoverageSampleStep_;
...
public:
    ...
    void SetJointAnglesDeg(const Vector6d &anglesDeg,
                           double baseX = 0,
                           double baseY = 0,
                           double baseZ = 0,
                           MotionMode mode = MotionMode::JOINT_ONLY,
                           MotionTaskType taskType = MotionTaskType::TRANSITION,
                           const Matrix4d* projectorPose = nullptr,
                           const std::array<Vector3d, 5>* frustumPoints = nullptr,
                           const std::vector<Vector3d>* coveredVertices = nullptr,
                                 const QString& planningInfo = QString());
    MotionTaskType currentMotionTaskType() const { return currentMotionTaskType_; }
    bool isCameraCaptureMotion() const { return currentMotionTaskType_ == MotionTaskType::CAMERA_CAPTURE; }
    bool isWeldingMotion() const { return currentMotionTaskType_ == MotionTaskType::WELDING; }
...
private:
    ...
    std::vector<MotionTaskType> recordedTrajectoryTypes_;  // 记录每段RRT轨迹的任务类型
    std::vector<Matrix4d> recordedProjectorPoses_;  // 记录拍照段的投影器位姿
    std::vector<bool> recordedHasProjectorPose_;  // 投影器位姿是否有效
    std::vector<std::array<Vector3d, 5>> recordedFrustumPoints_;  // 记录拍照段视锥点
    std::vector<std::vector<Vector3d>> recordedCoveredVertices_;  // 记录拍照段覆盖区域网格顶点
    std::vector<bool> recordedHasStructuredLightGeometry_;  // 是否有缓存几何
    std::vector<QString> recordedPlanningInfos_;  // 记录每段规划输出信息
    ...
    void setStructuredLightActorsVisible(bool visible);
    void updateStructuredLightVisualization();
    void updateHalfFrustumActors(const Matrix4d& projectorPose);
    void updateStructuredLightCoverage(const Matrix4d& projectorPose);
    void applyHalfFrustumActors(const std::array<Vector3d, 5>& frustumPoints, const Matrix4d& projectorPose);
    void applyStructuredLightCoveredGeometry(const std::vector<Vector3d>& coveredVertices);
    void buildHalfFrustumPoints(const Matrix4d& projectorPose, std::array<Vector3d, 5>& frustumPoints) const;
    void buildStructuredLightCoveredVertices(const Matrix4d& projectorPose,
                                             const std::array<Vector3d, 5>& frustumPoints,
                                             std::vector<Vector3d>& coveredVertices) const;
    bool isPointInsideHalfFrustum(const Vector3d& point, const Vector3d& position, const Vector3d& right, const Vector3d& up, const Vector3d& forward) const;
    bool doesTriangleAabbIntersectHalfFrustum(const Vector3d& bboxMin, const Vector3d& bboxMax, const std::array<Vector3d, 5>& frustumPoints) const;
    bool shouldShowCoveredSample(const Vector3d& p0,
                                 const Vector3d& p1,
                                 const Vector3d& p2,
    ...
    void addStructuredLightActorsToRenderers();
    void recordTrajectorySegment(const std::vector<Vector6d>& trajectory,
    ...
    QString makePlanningInfo(size_t segmentIndex,
                             MotionTaskType segmentType,
                             const Vector3d& basePosition,
                             double manipulability,
                             const Matrix4d* viewpointPose) const;
    void appendPlanningInfo(const QString& planningInfo);
